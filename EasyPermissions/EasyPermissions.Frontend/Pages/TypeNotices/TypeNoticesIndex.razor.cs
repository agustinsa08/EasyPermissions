...
namespace EasyPermissions.Frontend.Pages.TypeNotices
{
    ...
    public partial class TypeNoticesIndex
    {
        ...
        private async Task<bool> LoadListAsync(int page)
        {
            ...
            var responseHttp = await Repository.GetAsync<List<TypeNotice>>(url);
        ...
        {
            var url = "api/TypeNotices/totalPages";
            if (!string.IsNullOrEmpty(Filter))
            {
                url += $"?filter={Filter}";
            }
        ...
        private async Task DeleteAsycn(TypeNotice typeNotices)
        {
            var result = await SweetAlertService.FireAsync(new SweetAlertOptions
            {
                Title = "Confirmación",
                Text = $"¿Estas seguro de querer borrar la categoria de noticia: {typeNotices.Name}?",
            ...
            var responseHttp = await Repository.DeleteAsync<CategoryNotice>($"api/CategoryNotices/{typeNotices.Id}");
            if (responseHttp.Error)
            {
